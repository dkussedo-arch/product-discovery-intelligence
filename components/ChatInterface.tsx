...
export function ChatInterface() {
  ...
  return (
    <section className="flex h-[min(70vh,640px)] flex-col overflow-hidden rounded-2xl border border-[var(--color-card-border)] bg-[var(--color-card)] shadow-xl shadow-black/20">
      <div className="border-b border-[var(--color-card-border)] px-5 py-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-medium">Discovery chat</h2>
        </div>
        <p className="mt-1 text-sm text-[var(--color-muted)]">
          Ask follow-up questions — full conversation history is sent on each turn.
        </p>
      </div>
      ...
      <div className="flex-1 space-y-4 overflow-y-auto px-4 py-4 sm:px-5">
        {messages.length === 0 && !error && (
          <p className="text-center text-sm text-[var(--color-muted)]">
            Start a conversation about your product discovery work.
          </p>
        )}
      ...
      <div className="border-t border-[var(--color-card-border)] p-4">
        <div className="flex gap-3">
          <textarea
            ...
            placeholder="Ask about assumptions, research, or decisions…"
            rows={2}
            disabled={isGenerating}
          ...
          <button
            ...
            className="inline-flex h-12 w-12 shrink-0 items-center justify-center self-end rounded-xl bg-[var(--color-accent)] text-white transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-50"
