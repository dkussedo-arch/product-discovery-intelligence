...
import type { EvaluatedClaim, EvaluationResult, EvaluationStatus } from '@/lib/types'
...
function buildExportReport(results: EvaluationResult, metrics: ReturnType<typeof computeMetrics>): string {
  const lines = [
...
function ClaimRow({ claim }: { claim: EvaluatedClaim }) {
  ...
  const hasDetail = Boolean(claim.evidence || claim.rationale || claim.sources?.length)
    ...
    <div className="rounded-xl border border-[var(--color-card-border)] bg-[#0d1219]">
      ...
      {expanded && hasDetail && (
        <div className="space-y-3 border-t border-[var(--color-card-border)] px-4 py-4 text-xs text-[var(--color-muted)]">
          {claim.rationale && (
            <div>
              <p className="mb-1 font-medium uppercase tracking-wide text-[var(--color-foreground)]">
                Rationale
              </p>
...
export function EvaluationResults({ results }: EvaluationResultsProps) {
  ...
  const metrics = useMemo(() => computeMetrics(results.claims), [results.claims])

  const filteredClaims = useMemo(() => {
    if (statusFilter === 'ALL') {
      return results.claims
    }
    return results.claims.filter((claim) => claim.status === statusFilter)
  }, [results.claims, statusFilter])
    ...
    <section className="rounded-2xl border border-[var(--color-card-border)] bg-[var(--color-card)] p-6 shadow-xl shadow-black/20">
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-[1.2fr_1fr]">
        <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-6 py-5">
          ...
          <p className="mt-2 text-sm text-emerald-200/70">claims supported by evidence</p>
        </div>
